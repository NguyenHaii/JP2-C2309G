...
public class Main {
    public static void main(String[] args) {
        ...
        int orderId = 1;
        displayOrderDetail(orderId, orderDetails, products, "data/orderDetail.out.txt");
    }
    ...
    private static void displayOrderDetail(int orderId, List<OrderDetail> orderDetails, List<Product> products, String outputFilePath) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFilePath))) {
            writer.write("Order Details for Order ID: " + orderId + "\n");
            writer.write("+-----------------------------------------------------+\n");
            ...
            for (OrderDetail detail : orderDetails.stream().filter(od -> od.getOrderId() == orderId).collect(Collectors.toList())) {
                Product product = products.stream().filter(p -> p.getId() == detail.getProductId()).findFirst().orElse(null);
                if (product != null) {
